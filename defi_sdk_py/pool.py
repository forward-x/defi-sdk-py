...
class Pool:
    ...
    def active_rank(self, pool:IAPHPool, nft_id: int, is_estimate=False)->Union[TransactionReceipt, int]:
        contract_func = pool.activateRank(nft_id)
        return self.send_transaction(contract_func, is_estimate=is_estimate)

    def claim_all_interest(self, pool:IAPHPool, nft_id: int, is_estimate=False)->Union[TransactionReceipt, int]:
        contract_func = pool.claimAllInterest(nft_id)
        return self.send_transaction(contract_func, is_estimate=is_estimate)

    def claim_token_interest(self, pool:IAPHPool, nft_id: int, amount:int, is_estimate=False)->Union[TransactionReceipt, int]:
        token:IERC20Metadata = IERC20Metadata(pool.tokenAddress().call(), self.web3)
        amount = parseEther(self.web3, amount, token.decimals().call())
        contract_func = pool.claimTokenInterest(nft_id, amount)
        return self.send_transaction(contract_func, is_estimate=is_estimate)

    def claim_forw_interest(self, pool:IAPHPool, nft_id: int, amount:int, is_estimate=False)->Union[TransactionReceipt, int]:
    ...
    def borrow(self, pool:IAPHPool, nft_id:int, loan_id:int, borrow_amount:int, collateral_sent_amount:int, collateral_token:IERC20Metadata, is_estimate=False)->Union[TransactionReceipt, int]:
        ...
        return self.send_transaction(contract_func, value=collateral_sent_amount if is_native else 0, is_estimate=is_estimate)

    def open_position(self, nft_id:int, is_long:bool, collateral_token:IERC20Metadata, underlying_token:IERC20Metadata, entry_price:int, size:int, leverage:int, slip_page:int, is_estimate=False):
